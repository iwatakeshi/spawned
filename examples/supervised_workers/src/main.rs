...
impl Actor for FlakyWorker {
    async fn started(&mut self, _ctx: &Context<Self>) {
        ...
        if n == 0 {
            tracing::warn!("[{}] crashing on first start — supervisor should restart me", self.name);
...
impl StableWorker {
    ...
    #[request_handler]
    async fn handle_ping(&mut self, _msg: stable_worker_protocol::Ping, _ctx: &Context<Self>) -> String {
