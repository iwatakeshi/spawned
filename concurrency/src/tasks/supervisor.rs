...
#[cfg(feature = "cluster")]
use crate::cluster::{
    remote_spawn_spec_from_inner, request_spawn_async, shutdown_remote, RemoteChildHandle,
    RemoteSpawnMeta,
};
#[cfg(feature = "cluster")]
...
impl Supervisor {
    ...
    async fn start_child_remote(
        ...
        let address = match request_spawn_async(&placement, parent.clone(), wire_spec.clone(), true).await {
            Ok(address) => address,
            Err(err) => {
    ...
    async fn restart_remote_child(&mut self, ctx: &Context<Self>, child_id: &str) {
        ...
        let parent = ctx.actor_address();
        let address = match request_spawn_async(
            &meta.placement,
            parent.clone(),
            meta.spec.clone(),
            meta.link,
