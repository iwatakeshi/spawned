...
use crate::child_spec::{
    shutdown_child_blocking, RestartBackoff, RestartIntensity, ShutdownType,
};
...
#[cfg(feature = "cluster")]
use crate::cluster::{
    request_spawn_with_retry_blocking, shutdown_remote_and_wait_blocking, RemoteChildHandle,
    RemoteSpawnMeta, RemoteSpawnRetryPolicy,
};
#[cfg(feature = "cluster")]
...
impl DynamicSupervisorApi for ActorRef<DynamicSupervisor> {
    fn start_child(
        ...
        Response::ready(self.request(StartChild { spec, reg_name }))
    }

    fn terminate_child(&self, actor_id: ActorId) -> Response<Result<(), DynamicSupervisorError>> {
        Response::ready(self.request(TerminateChild { actor_id }))
    }

    fn count_children(&self) -> Response<usize> {
        Response::ready(self.request(CountChildren))
    }
    ...
    fn terminate_child_by_actor(
        ...
        if let Some(child_id) = self.remote_actor_to_id.get(&actor_id).cloned() {
            if let Some(remote) = self.remote_handles.remove(&child_id) {
                ...
                shutdown_remote_and_wait_blocking(&remote, shutdown).map_err(|e| {
                    DynamicSupervisorError::RemoteSpawn(e.to_string())
                })?;
...
impl Handler<TerminateChild> for DynamicSupervisor {
    fn handle(
        ...
        let _ = ctx;
        self.terminate_child_by_actor(msg.actor_id)
    }
}

impl Handler<CountChildren> for DynamicSupervisor {
    fn handle(&mut self, _msg: CountChildren, _ctx: &Context<Self>) -> usize {
        self.logic.child_count()
    }
}

impl Handler<WhichChildren> for DynamicSupervisor {
    fn handle(&mut self, _msg: WhichChildren, _ctx: &Context<Self>) -> Vec<DynamicChildInfo> {
        self.logic
            .list_children()
            .into_iter()
            .map(|child| {
...
mod tests {
    ...
    fn start_count_which_and_terminate() {
        ...
        let children = sup.which_children().unwrap();
        assert_eq!(children.len(), 1);
        assert!(children[0].id.starts_with("w#"));

        sup.terminate_child(handle.id()).unwrap().unwrap();
        assert_eq!(sup.count_children().unwrap(), 0);
    ...
    fn registry_name_is_registered_and_unregistered() {
        ...
        let children = sup.which_children().unwrap();
        sup.terminate_child(children[0].actor_id).unwrap().unwrap();
