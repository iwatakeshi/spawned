//! Static supervisor remote_named: supervisor on A, named-spec child on B.
...
use spawned_concurrency::tasks::{register_remote_spec, Actor, Context, ChildSpec, Supervisor};
...
#[tokio::test]
async fn static_supervisor_remote_named_restarts_on_exit() {
    ...
    *REMOTE_ACTOR_ID.lock().unwrap_or_else(|p| p.into_inner()) = None;
    ...
    register_remote_spec("test.Counter/v1", || {
        ChildSpec::worker("counter", || Counter { value: 0 }, RestartType::Permanent)
    })
    .unwrap();
